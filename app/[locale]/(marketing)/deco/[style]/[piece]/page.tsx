...
export default async function DecoStylePiecePage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[350px] md:h-[450px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full z-10">
          <div className="container mx-auto px-4 pb-10">
            {/* Breadcrumb */}
            <nav className="flex items-center gap-1.5 text-sm text-[var(--mist)] mb-6">
              <Link href="/" className="hover:text-[var(--gold)] transition-colors">Accueil</Link>
              <ChevronRight className="h-3 w-3" />
              <Link href={`/style/${style.slug}`} className="hover:text-[var(--gold)] transition-colors">{style.name}</Link>
              <ChevronRight className="h-3 w-3" />
              <Link href={`/piece/${room.slug}`} className="hover:text-[var(--gold)] transition-colors">{room.name}</Link>
            </nav>

            <div className="flex flex-wrap gap-2 mb-4">
              <Badge className="bg-[var(--gold)] text-[#0c0a09] border-none">
                <Palette className="h-3 w-3 mr-1" />
                {style.name}
              </Badge>
              <Badge className="bg-[rgba(200,162,77,0.12)] text-[var(--ivory)] border-[var(--gold-line)] backdrop-blur-sm">
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {style.colors.map(color => (
                      <span key={color} className="bg-[rgba(200,162,77,0.12)] text-foreground px-3 py-1.5 rounded-full text-sm border border-[var(--gold-line)]">
                        {color}
                      </span>
                    ))}
                  </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {style.materials.map(mat => (
                      <span key={mat} className="bg-[rgba(200,162,77,0.12)] text-foreground px-3 py-1.5 rounded-full text-sm border border-[var(--gold-line)]">
                        {mat}
                      </span>
                    ))}
                  </div>
      ...
      {/* Maillage interne */}
      <section className="py-12 border-t border-[var(--gold-line)] bg-card/40">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="prestige-reveal flex flex-wrap gap-4 text-sm">
            ...
            <Link href={`/piece/${room.slug}`} className="text-[var(--gold)] hover:underline">
              Guide complet : {room.name} →
            </Link>
            <Link href="/galerie" className="text-[var(--gold)] hover:underline">
              Voir la galerie →
            </Link>
          </div>
        </div>
