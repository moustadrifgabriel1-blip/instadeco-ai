...
- ✅ **Déployé** : titles+metas CTR dédiés des 8 villes indexées (map `CITY_SEO_OVERRIDES` en code, fallback ENTRE l'override cron `seo_title_overrides` et le gabarit, la table reste intacte) ; `/pro` fr-only (canonical /fr/pro + noindex en/de, sitemap fr-only, priorité 0.9) ; pivot d'intention des pages villes (H1 « Home staging virtuel et déco par IA à {ville} », maillage vers /solution/home-staging-virtuel-agents-immobiliers + /pro) ; double suffixe de marque retiré (le template layout ajoute déjà `| InstaDeco AI`, on enlève le `| InstaDeco` manuel des metaTitles /solution + du title /amenager) ; CTR `/solution/logiciel-home-staging` recentré sur « logiciel de home staging à partir d'une photo » (requête réelle pos ~11) ; H1 `/pro` réparé (espace manquant) ; tirets cadratins purgés du copy visible (intent-pages-data, programmatic-data) ; affirmation fausse « technologie ControlNet » reformulée.
- ✅ **E-E-A-T blog** : auteur humain nommé **Gabriel Moustadrif** (byline + rôle « Fondateur d'InstaDeco » + schema BlogPosting `author` = `Person` lié à /a-propos, au lieu d'Organization). NB : `lib/seo/config.ts` `founders` reste `['InstaDeco Team']` (champ non émis dans le schema Organization, à brancher si on veut un `founder` Person).
- ✅ **Barrière qualité pSEO `/amenager`** (anti scaled-content) : helpers `countWords`/`isPseoContentIndexable` (`pseo-quality.ts`), `noindex,follow` tant que `unique_seo_text` < 120 mots + exclusion sitemap, exactement comme les villes. Mesure base : les 56 pages font ~72 mots uniques (max ~99) → toutes passent en noindex, réindexation auto dès enrichissement.
- 🔴 **Findings mesurés à traiter (URL Inspection API)** : `/fr/pro` **NON indexée** (statut « Alternate page with proper canonical tag », dernier crawl 22/05 AVANT le fix fr-only → devrait se résoudre, à forcer via « Demander l'indexation » GSC UI) ; des **articles blog « inconnus de Google »** (découverte faible, maillage interne à renforcer) ; pages `/amenager` jamais indexées (confirme que le noindex coûte 0 trafic) ; crawl rare (~2 à 4 semaines, site jeune faible autorité).
- ⏳ **Restants (non bloquants)** : **Option 2 pSEO DÉPRIORISÉE** (les /amenager sont inconnues de Google = aucune demande, ne pas y investir de contenu) ; **PR6 maillage interne** vers /pro et /solution/logiciel-home-staging (aide indexation money page + découverte) ; propagation 10s→30s (cf. mémoire `delai-generation-incoherent`) ; CWV terrain (besoin clé PageSpeed, mais trafic trop faible pour CrUX) ; backlinks (non exposés par l'API GSC, UI/outil seulement) ; cluster de contenu agents immo (home staging virtuel, vendre plus vite, comparatif virtuel/physique). Vérifié : aucun spam/abus (corrections de pages existantes uniquement, empreinte d'index réduite, white-hat).
